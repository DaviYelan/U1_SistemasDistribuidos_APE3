...
def run_panaderia_simulation(emit_fn, done_fn, capacidad=5, n_panes=10, n_clientes=3):
    ...
    while panes_consumidos[0] < n_panes and time.time() < deadline:
        time.sleep(1.5)
    ...
    done_fn({
        ...
        "conclusion": (
            f"✅ Productor-Consumidor correcto. Producidos={panes_producidos[0]}, "
            f"Consumidos={panes_consumidos[0]}"
            if correcto else
